...
This repo is now a **React** app (Vite + React Router). All content from the original HTML/CSS/JS site has been copied into React components.

- **Run locally:** `npm install` then `npm run dev`
- **Build:** `npm run build` → output in `dist/`
- **Preview build:** `npm run preview`

Assets are served from `public/assests/` (your original `assests` folder was copied there). For the optional chatbot AI (DeepSeek), set `VITE_DEEPSEEK_API_KEY` in a `.env` file.

The original static files (`about.html`, `services.html`, `projects.html`, `contact.html`, `style.css`, `app.js`) remain in the repo for reference.
...
Phone: +91 9339828230
LinkedIn: Kobayashi Issa ( username : @mdaffanasghar)
